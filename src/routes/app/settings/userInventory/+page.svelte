<script>
  ...
  try {
    ...
    // Đảm bảo nếu không có role, thì mặc định là 'viewer'
    const role = "viewer"; // Vai trò mặc định

    const response = await apiFetch("/user-inventory/add-by-email", {
      method: "POST",
      body: {
        email: newUserEmail,
        status: "active", // Trạng thái người dùng mặc định là 'active'
        permission: 1,    // Quyền mặc định là 1 (có quyền)
        role: role,       // Gửi role mặc định
      },
      headers: {
        'x-type': 'users'
      }
  ...
  try {
    ...
    const response = await apiFetch(`user-inventory/remove/${userId}`, {
      method: "DELETE",
